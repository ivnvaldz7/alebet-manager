...
async function seedProductos() {
  try {
    ...
    if (count > 0) {
      console.log(`✅ Ya hay ${count} productos en la base de datos`)
      console.log('⚠️  Ejecuta "npm run seed:productos:force" para borrar y recrear')
      process.exit(0)
    }

    // Productos basados en el PDF de DEPÓSITO.pdf
    const productos = [
      // OLIVITASAN
      {
        nombre: 'OLIVITASAN',
    ...
    // Generar nombreCompleto para cada producto
    const productosConNombre = productos.map((p) => ({
      ...p,
      nombreCompleto: p.variante
        ? `${p.nombre} ${p.variante} ${p.presentacion}`
        : `${p.nombre} ${p.presentacion}`,
    }))

    await ProductModel.insertMany(productosConNombre)

    console.log(`✅ ${productosConNombre.length} productos creados exitosamente:`)
    productos.forEach((p) => {
      console.log(`   - ${p.nombre} ${p.variante || ''} ${p.presentacion}`)
    })
